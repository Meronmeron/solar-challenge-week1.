...
===============================================
This script performs comprehensive EDA on the Togo (Dapaong) solar dataset.
"""
...
if timestamp_col:
    ...
    plt.tight_layout()
    plt.savefig('plots/togo_radiation_time_series.png')
    plt.close()
    ...
    if 'Tamb' in df_clean.columns:
        ...
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig('plots/togo_temperature_time_series.png')
        plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('plots/togo_hourly_radiation.png')
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('plots/togo_monthly_radiation.png')
    plt.close()
...
if 'GHI' in df_clean.columns:
    ...
    if 'WS' in df_clean.columns:
        ...
        plt.savefig('plots/togo_ws_ghi_scatter.png')
        plt.close()
    ...
    if 'RH' in df_clean.columns:
        ...
        plt.tight_layout()
        plt.savefig('plots/togo_rh_relationships.png')
        plt.close()
...
plt.tight_layout()
plt.savefig('plots/togo_distributions.png')
plt.close()
...
if 'RH' in df_clean.columns and 'Tamb' in df_clean.columns and 'GHI' in df_clean.columns:
    ...
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('plots/togo_rh_effect.png')
    plt.close()
