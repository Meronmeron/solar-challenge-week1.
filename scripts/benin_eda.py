...
=================================================
This script performs comprehensive EDA on the Benin (Malanville) solar dataset.
"""
...
if timestamp_col:
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('plots/benin_monthly_radiation.png')
    plt.close()
...
if 'GHI' in df_clean.columns:
    ...
    if 'RH' in df_clean.columns:
        ...
        plt.tight_layout()
        plt.savefig('plots/benin_rh_relationships.png')
        plt.close()
...
plt.tight_layout()
plt.savefig('plots/benin_distributions.png')
plt.close()
...
if 'RH' in df_clean.columns and 'Tamb' in df_clean.columns and 'GHI' in df_clean.columns:
    ...
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('plots/benin_rh_effect.png')
    plt.close()
